...
void UsartForm::initOrderList()
{
    orderList<<"a1b"<<"a2b"<<"a3b"
             ...
             <<"aAb"<<"aBb"<<"aCb"<<"aDb";
    orderList<<"aEb";//ok
    funcList<<"打开LED1"<<"关闭LED1"<<"打开LED2"<<"关闭LED2"<<"打开蜂鸣器"<<"关闭蜂鸣器"<<"获取锂电池电压"
            <<"获取温湿度"<<"获取光线亮度"<<"获取大气压压强"<<"获取电位器电压"<<"获取CPU温度"<<"获取RFID卡ID"
            <<"发送OK按键";
    MSG_N.clear();
}
...
void UsartForm::on_checkBox_beep_stateChanged(int arg1)
{
    if(arg1>0)
    {
        ...
        emit sendSTM32Data(orderList.at(4).toUtf8());
         ui->checkBox_beep->setIcon(QIcon(":/image/images/beep1.png"));
    }
    else
    {
        //beep off
        ...
        ui->checkBox_beep->setIcon(QIcon(":/image/images/beep2.png"));
    }
}
